        ...
        <DashboardLayout>
            ...
            <div className="space-y-10 mt-8">
                ...
                <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
                    <div>
                        <h2 className="text-3xl font-black text-gray-900 dark:text-white">Financial Insights</h2>
                        <p className="text-sm text-gray-500 dark:text-zinc-400 mt-1">Deep dive into your spending habits and revenue streams.</p>
                    </div>
                    <div className="flex items-center gap-3">
                        <div className="bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 p-1.5 rounded-2xl flex items-center shadow-sm">
                            <button className="px-4 py-2 bg-gray-50 dark:bg-zinc-800 rounded-xl text-xs font-bold text-gray-900 dark:text-zinc-100">Monthly</button>
                            <button className="px-4 py-2 text-xs font-bold text-gray-400 hover:text-black dark:hover:text-white">Yearly</button>
                        </div>
                        <button className="bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 p-3 rounded-2xl text-gray-400 hover:text-black dark:hover:text-white transition-colors shadow-sm">
                            <Filter className="w-5 h-5" />
                        </button>
                        <button className="bg-black dark:bg-white text-white dark:text-black px-8 py-3 rounded-2xl font-black text-sm flex items-center gap-2 hover:scale-105 transition-all shadow-xl shadow-black/10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <div className="bg-white dark:bg-zinc-900 p-8 rounded-[40px] shadow-sm border border-gray-50 dark:border-zinc-800 transition-colors">
                        ...
                        <p className="text-3xl font-black text-gray-900 dark:text-white">${stats.totalIncome.toLocaleString()}</p>
                    ...
                    <div className="bg-white dark:bg-zinc-900 p-8 rounded-[40px] shadow-sm border border-gray-50 dark:border-zinc-800 transition-colors">
                        ...
                        <p className="text-3xl font-black text-gray-900 dark:text-white">${stats.totalExpenses.toLocaleString()}</p>
                        <div className="mt-4 flex items-center gap-1 text-red-500 font-bold text-[10px]">
                            <TrendingDown className="w-3 h-3" /> 4% Increase
                        </div>
                    </div>
                    <div className="bg-[#FAF9F6] dark:bg-zinc-900 p-8 rounded-[40px] border border-gray-100 dark:border-zinc-800 flex flex-col justify-between transition-colors">
                        <div className="flex justify-between items-start">
                            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Savings Rate</p>
                            <Target className="w-4 h-4 text-purple-500" />
                        </div>
                        <p className="text-3xl font-black text-gray-900 dark:text-white">{stats.savingsRate.toFixed(1)}%</p>
                        <div className="mt-4 h-1.5 bg-gray-200 dark:bg-zinc-700 rounded-full overflow-hidden">
                        ...
                        <div className="flex-1 flex flex-col justify-center">
                            <div className="h-[220px] w-full relative">
                                ...
                                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                                    ...
                                    <p className="text-xl font-black text-gray-900 dark:text-zinc-100">${stats.totalExpenses.toLocaleString()}</p>
                                </div>
                            </div>
                            <div className="mt-8 space-y-3">
                                {pieData.map((data, i) => (
                                    <div key={i} className="flex items-center justify-between p-2 rounded-xl hover:bg-gray-50 dark:hover:bg-zinc-800 transition-colors">
                                        <div className="flex items-center gap-3">
                                            <div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: COLORS[i % COLORS.length] }} />
                                            <span className="text-[10px] font-bold text-gray-500 dark:text-zinc-400 uppercase tracking-widest">{data.name}</span>
                                        </div>
                                        <span className="text-[10px] font-black text-gray-900 dark:text-zinc-100">${data.value.toLocaleString()}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 pb-12">
                    ...
                    <div className="lg:col-span-2 bg-white dark:bg-zinc-900 p-10 rounded-[40px] shadow-sm border border-gray-50 dark:border-zinc-800 transition-colors">
                        <div className="flex items-center justify-between mb-8">
                            ...
                            <div className="relative">
                                ...
                                <input type="text" placeholder="Search reports..." className="pl-8 pr-4 py-1.5 bg-gray-50 dark:bg-zinc-800 dark:text-zinc-300 border-none rounded-lg text-[10px] focus:ring-black dark:focus:ring-white w-40" />
                            </div>
                        </div>
                            ...
                            <table className="w-full text-left">
                                ...
                                <tbody>
                                    {recentReports.map((report) => (
                                        <tr key={report.id} className="border-b border-gray-50 dark:border-zinc-800 group hover:bg-gray-50/50 dark:hover:bg-zinc-800/40 transition-colors">
                                            <td className="py-5">
                                                <div className="flex items-center gap-4">
                                                    <div className="w-10 h-10 rounded-xl bg-gray-100 dark:bg-zinc-800 flex items-center justify-center text-gray-500 dark:text-zinc-400 transition-colors">
                                                        <FileText className="w-5 h-5" />
                                                    </div>
                                                    <div>
                                                        <p className="text-xs font-bold text-gray-900 dark:text-zinc-100">{report.name}</p>
                                                        <p className="text-[9px] font-black text-gray-400 uppercase">ID: {report.id}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="py-5 text-center">
                                                <span className="px-3 py-1 rounded-lg bg-gray-100 dark:bg-zinc-800 text-[10px] font-black text-gray-600 dark:text-zinc-300">{report.type}</span>
                                            </td>
                                            <td className="py-5 text-center">
                                                <span className="text-[10px] font-bold text-gray-400 uppercase">{new Date(report.date).toLocaleDateString()}</span>
                                            </td>
                                            <td className="py-5 text-right">
                                                <button className="p-2 hover:bg-black dark:hover:bg-white hover:text-white dark:hover:text-black rounded-xl transition-all">
                    ...
                    <div className="space-y-8">
                        <div className="bg-[#FAF9F6] dark:bg-zinc-900 p-8 rounded-[40px] border border-gray-100 dark:border-zinc-800 transition-colors">
                            ...
                            <div className="space-y-6">
                                <div>
                                    <div className="flex justify-between items-end mb-2">
                                        <span className="text-[10px] font-black text-gray-500 dark:text-zinc-400 uppercase tracking-widest">Entertainment</span>
                                        <span className="text-[10px] font-black text-gray-900 dark:text-zinc-100">$850 / $1,000</span>
                                    </div>
                                    <div className="h-1.5 bg-gray-200 dark:bg-zinc-700 rounded-full overflow-hidden">
                                        <motion.div initial={{ width: 0 }} animate={{ width: '85%' }} className="h-full bg-black dark:bg-white" />
                                    </div>
                                </div>
                                <div>
                                    <div className="flex justify-between items-end mb-2">
                                        <span className="text-[10px] font-black text-gray-500 dark:text-zinc-400 uppercase tracking-widest">Shopping</span>
                                        <span className="text-[10px] font-black text-gray-900 dark:text-zinc-100">$2,400 / $2,000</span>
                                    </div>
                                    <div className="h-1.5 bg-gray-200 dark:bg-zinc-700 rounded-full overflow-hidden">
